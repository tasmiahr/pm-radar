...
def scrape_greenhouse(slug: str, company: str, tier: int, work_pref: str) -> list[dict]:
    ...
    for j in data["jobs"]:
        ...
        # Greenhouse exposes compensation_range on some listings
        comp = j.get("compensation_range") or {}
        if comp:
            min_c = comp.get("min_cents", 0) or 0
            max_c = comp.get("max_cents", 0) or 0
            curr  = comp.get("currency_type", "USD")
            if min_c and max_c:
                structured_sal = f"{curr} ${min_c//100:,} - ${max_c//100:,}"
            elif min_c:
                structured_sal = f"{curr} ${min_c//100:,}+"
            else:
                structured_sal = None
        else:
            structured_sal = None
